...
export const StaffAdminMessagesPage = () => {
  ...
  const { messages, isTyping, onlineMap, sendMessage, emitTyping } =
    useStaffAdminChat(selectedAdminId);
      ...
      <div className="flex-1 overflow-y-auto p-2">
        ...
        {!loading && !loadError && adminList.length === 0 && (
          <div className="space-y-2 px-2 py-4 text-xs text-muted-foreground">
            ...
            <Button type="text" size="small" className="h-8 gap-1.5 px-2" onClick={() => void fetchAdminList()}>
              <RefreshCw className="h-3.5 w-3.5" />
              Tải lại
            </Button>
          </div>
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] min-h-[420px] w-full overflow-hidden rounded-xl border border-border bg-card">
      {isMobile ? (
        ...
        <div className="flex w-[280px] shrink-0 flex-col border-r border-border bg-muted/30">{adminPanel}</div>
      )}
            ...
            {selectedAdminId ? (
              <>
                ...
                <div className="min-w-0">
                  <div className="truncate font-semibold">{selected?.name || `Admin #${selectedAdminId}`}</div>
