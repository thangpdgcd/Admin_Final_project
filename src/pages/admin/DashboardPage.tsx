...
// Note: we only need `toMonthKey` + `toDateKey` for analytics bucketing.

export function DashboardPage() {
  const [error, setError] = useState<string | null>(null);
  const [latestOrders, setLatestOrders] = useState<Array<{ id: string; customer: string; total: number; status: string; createdAt?: string }>>([]);
  const [latestUsers, setLatestUsers] = useState<Array<{ id: string; name: string; email: string; role: string; createdAt?: string }>>([]);
        ...
        setLatestOrders(
          ordersResult.items.slice(0, 5).map((order, index) => ({
            ...
            customer:
              order.user?.name ||
              order.user?.email ||
              ((order as Record<string, unknown>).users as { name?: string; email?: string } | undefined)?.name ||
              ((order as Record<string, unknown>).users as { name?: string; email?: string } | undefined)?.email ||
              "N/A",
            total: Number(
            ...
            status: String(order.status || (order.isDelivered ? "delivered" : order.isPaid ? "confirmed" : "pending")),
  ...
  const renderedAnalyticsSeries = useMemo(() => analyticsByMode[analyticsMode] ?? [], [analyticsByMode, analyticsMode]);

  const latestOrdersColumns = useMemo<ColumnsType<(typeof latestOrders)[number]>>(
    () => [
      { title: "Customer", dataIndex: "customer", key: "customer" },
      { title: "Total", dataIndex: "total", key: "total", render: (v: number) => formatCurrency(Number(v || 0)) },
      {
        title: "Status",
        dataIndex: "status",
        key: "status",
        render: (v: string) => {
          ...
          const color = s.includes("deliver") ? "blue" : s.includes("paid") || s.includes("confirm") ? "green" : "orange";
          const label =
            s.includes("deliver") ? "Đã giao" : s.includes("paid") || s.includes("confirm") ? "Đã thanh toán" : "Chưa thanh toán";
  ...
  const latestUsersColumns = useMemo<ColumnsType<(typeof latestUsers)[number]>>(
    () => [
      ...
      { title: "Role", dataIndex: "role", key: "role", render: (v: string) => <Tag>{String(v)}</Tag> },
    ],
    []
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Overview of users, products, orders, and revenue.</p>
      </div>

      {error ? <p className="text-sm text-destructive">{error}</p> : null}

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card><Statistic title="Total users" value={summary.totalUsers} /></Card>
        <Card><Statistic title="Total products" value={summary.totalProducts} /></Card>
        <Card><Statistic title="Total orders" value={summary.totalOrders} /></Card>
        <Card><Statistic title="Revenue" value={formatCurrency(revenue)} /></Card>
      </div>

      <Card>
        <div className="flex items-start justify-between gap-3">
          <div>
            <Typography.Title level={5} className="mb-0!">Activity Analytics</Typography.Title>
            ...
            <div className="h-[260px] rounded-xl border border-border/60 bg-background/20 px-2 pt-2">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={renderedAnalyticsSeries} margin={{ top: 8, right: 12, bottom: 8, left: 4 }}>
                  ...
                  <Area type="monotone" name="Orders" dataKey="orders" stroke="#6366f1" fill="url(#ordersGradient)" strokeWidth={2.5} dot={false} />
                  <Area type="monotone" name="Users" dataKey="users" stroke="#a855f7" fill="url(#usersGradient)" strokeWidth={2.5} dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <Typography.Title level={5} className="mb-4!">Latest orders</Typography.Title>
          <Table
            rowKey="id"
        ...
        <Card>
          <Typography.Title level={5} className="mb-4!">Latest users</Typography.Title>
          <Table
            rowKey="id"
