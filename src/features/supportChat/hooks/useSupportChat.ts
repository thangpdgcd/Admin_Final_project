...
export const useSupportChat = () => {
  ...
  const resolvedRole = resolveUserRole(user as unknown as Record<string, unknown>)
  ...
  const loadConversations = React.useCallback(async () => {
    ...
    try {
      ...
      const convs = dtos
        .map((d) => normalizeConversation(d, { currentUserId: userId }))
    ...
    async (conversationId: number) => {
      ...
      try {
        ...
        const ui = msgs
          ...
          .map((m) => normalizeMessage(m, { direction: guessDirection(m, userId) }))
  ...
  const connectSocket = React.useCallback(() => {
    ...
    const handleIncoming = (payload: unknown) => {
      ...
      const ui = normalizeMessage(dto, { direction: guessDirection(dto, userId) })

      // Optimistic reconciliation: replace a pending outgoing with same text.
  ...
  const sendTextMessage = React.useCallback(
    (args: { conversationId?: number; recipientUserId?: number; text: string }) => {
      ...
      const optimistic: UiMessage = {
        ...
        type: "text",
        text,
        senderRoleId: resolvedRole === "staff" ? STAFF_ROLE_ID : undefined,
        createdAtMs: now,
        timeLabel: toTimeLabel(now),
        direction: "outgoing",
        status: "sending",
      ...
      try {
        ...
        // Backend persists on `chat:message`.
        if (convId != null) {
          s.emit("chat:message", {
            conversationId: convId,
    ...
    [resolvedRole, selectedConversationId, store],
  )
