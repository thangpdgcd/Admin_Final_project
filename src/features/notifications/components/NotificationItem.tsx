...
const NotificationIcon = ({ type, isRead }: { type: NotificationRow["type"]; isRead: boolean }) => {
  const Icon =
    type === "order"
      ? ShoppingCart
      : type === "chat"
...
export const NotificationItem = ({ item, onClick }: { item: NotificationRow; onClick: () => void }) => {
  return (
    <button
      type="button"
      onClick={onClick}
